(function ($) {
  ...
  var buildQuiz = function (input) {
    ...
    $(".quiz-container").html("<div class='progress'>Pregunta " + qnumber + "&nbsp;de&nbsp;" + input.length + "</div><div class='qq-question'>" + "<div class='question'>" + input[currentQuestion].question + "</div></div>" +
      "<ol class='answers'><li id='option-a'>" + input[currentQuestion].a + "</li>" +
      "<li id='option-d'>" + input[currentQuestion].b + "</li></ol>" +
      "<button class='qq-button submit-answer'>Enviar respuesta</button>" +
  ...
  var checkAnswer = function () {
    if ($(".selected").length > 0) {
      ...
      if (answer == input[currentQuestion].answer) {
        ...
        score++;
        displayProgress();
        $(".answer").html("<p>&iexcl;Correcto!</p><p>" + input[currentQuestion].correct + "</p>");

      } else {
        trackEvent(
          'q' + qnumber + '-answered-incorrectly',
          'Q' + qnumber + ' answered incorrectly');
        $(".answer").html("<p>&iexcl;No!</p><p> " + "&nbsp;La respuesta correcta es " + input[currentQuestion].answer + "<br><br>" + input[currentQuestion].incorrect + "</p>");
      }
      if (currentQuestion != (input.length-1)) {
        $(".answer").append("<button class='qq-button next'>Siguiente</button>");
        $('.next').on('click', nextQuestion);
      } else {
        $(".answer").append("<button class='qq-button check-score'>Ver resultados</button>");
  ...
  var finalScore = function () {
    ...
    $(".quiz-container")
      .html("<div class='scorecard'><p>Usted contest&#243; correctamente</p><p>" + score + "&nbsp;de&nbsp;" + input.length + "</p><div id='social-media'><ul><li><a class=\"fb-share\" href='http://www.facebook.com/sharer.php?u=" + link + "' target='_blank'>" + facebook + "</a></li><li><a class=\"twitter-share\" href='http://twitter.com/home?status=¡Inténtalo!, yo contesté correctamente " + score + "/" + input.length + " en esta trivia de Mayweather-Pacquiao " + link + " por @" + account + "' target='_blank'>" + twitter   + "</a></li><li><a class=\"gplus-share\" href='https://plus.google.com/share?url=" + link + "' target='_blank'>" + google + "</a></li></ul></div><p>&iexcl;Reta a tus amigos!</p></div>");
