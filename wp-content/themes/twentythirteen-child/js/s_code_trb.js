...
/************************** GLOBAL VAR SECTION **************************/
var globalCurDate = new Date();

/* getYear or getFullYear depending on browser app */
var appVers = parseInt(navigator.appVersion, 10);
var appName = navigator.appName;
if((appVers == '2' || appVers == '3') && (appName == 'Netscape' || appName == 'Microsoft Internet Explorer')) {
    var TheYear = globalCurDate.getYear();
        if (TheYear >= 100 && TheYear <= 1999) {
            var globalCurYear=TheYear + 1900
        }
        else {
                var globalCurYear=TheYear
        }
}
else{
        var globalCurYear = globalCurDate.getFullYear();
}
...
s.linkDownloadFileTypes="exe,zip,mp3,doc,pdf,xls,acrobat,xml"
s.linkInternalFilters="javascript:,"+ window.location.host
s.linkLeaveQueryString=false
s.linkTrackVars="server"
s.linkTrackEvents="None"
// Begin PN Plugin Params
s.siteID=""; // optional if we have no pagename, use this.
s.defaultPage="";
...
s.dstStart="1/1/" + globalCurYear;
s.dstEnd="1/1/" + globalCurYear;
s.currentYear=globalCurYear;
...
s.eVar25=s.getDaysSinceLastVisit();

cct=globalCurDate.getTime();
var month = globalCurDate.getMonth();
var day = globalCurDate.getDate();
var mon=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
var thism=mon[month];var currentDate = (thism + globalCurYear);String(currentDate);


if ((!window.s.pageType) && (!window.s.pageName || s.pageName==""))s.pageName=s.getPageName();
/* Check for Paid Search */
s.campaign = s.getQueryParam('gcid,track,coll,ctrack,cmp','');

/* Cookie Check for Pathing Reports */
var tc = "current";if(document.cookie.indexOf('s_path')==-1) {
s.c_w("s_path",tc,0);s.prop30 = "Y"}else{s.prop30 = "N"}

/* Set event 5 (page view) on every page  */    
s.events=s.apl(s.events,'event5',',','1');

/* Plugin: timeparting CST */
s.prop33=s.getTimeParting('d','-6'); // Set day 
s.prop34=s.getTimeParting('h','-6'); // Set hour 
s.prop35=s.getTimeParting('w','-6'); // Set weekday 


/* Repeat Visitor by campaign - Added by Mark Stringham Omniture IC */
      
var temp1=s.getQueryParam('gcid');var temp2=s.getQueryParam('track');var temp3=s.getQueryParam('ctrack');
var temp4=s.getQueryParam('coll');var temp5=s.getQueryParam('cmp');var temp6;
if(temp1){temp6=currentDate+':'+'gcid:'+temp1};if(temp2){temp6=currentDate+':'+'track:'+temp2};if(temp3){temp6=currentDate+':'+'ctrack:'+temp3};
if(temp4){temp6=currentDate+':'+'coll:'+temp4};if(temp5){temp6=currentDate+':'+'cmp:'+temp5};
var isFtcv = s.c_r('s_ftcv');var isVisit = s.c_r('s_current');
var ex=new Date();ct=ex.getTime();ex.setTime(ct+90*24*60*60*1000) // 90 days expire;
if((temp6) && (!isFtcv)){s.c_w('s_ftcv',temp6,ex);s.c_w('s_current',temp6,0);
s.events=s.apl(s.events,'event9',',','1');s.eVar26=temp6};
if((isFtcv) && (!isVisit)){s.events=s.apl(s.events,'event10',',','1')}//Set Repeat visit event;


/* End Repeat Visitor by campaign */  

/* navigation tracking */
s._lid=s.getQueryParam('lid','','',1)
s._lpos=s.getQueryParam('lpos','','',1)
s._previousPage=s.getPreviousValue(s.pageName,'gpv_pp','');
if(s._lid&&s._lpos){
    s._checkValue=s._lid+s._lpos+s._previousPage;
    s._checkValue=s.getValOnce(s._checkValue,'s_linkTrack',30);
    if(s._checkValue){
        s.prop47=s._previousPage;
        s.prop48=s._lid;
        s.prop49=s._lpos;
    }
}

s.prop65=s.getPreviousValue(s.prop38,'gpv_p1','event5');
...
/*
 * Plugin: getDaysSinceLastVisit v1.2 
 */
s.getDaysSinceLastVisit=new Function("c",""
+"var s=this,e=new Date(),a=new Date(),b,f,g,h=e.getTime(),i=86400000"
+",j='Cookies Not Supported',k='First Visit',l='More than 30 days',m="
+"'More than 7 days',n='Less than 7 days',o='Less than 1 day',d;e.set"
+"Time(h+94608000000);a.setTime(h+1800000);c=c?c:'s_dslv';b=s.c_r(c);"
+"if(b.length==0){s.c_w(c,h,e);s.c_w(c+'_s',k,a);}else{d=h-b;if(d>30*"
+"60*1000){if(d>30*i){s.c_w(c,h,e);s.c_w(c+'_s',l,a);}else if(d<30*i+"
+"1&&d>7*i){s.c_w(c,h,e);s.c_w(c+'_s',m,a);}else if(d<7*i+1&&d>i){s.c"
+"_w(c,h,e);s.c_w(c+'_s',n,a);}else if(d<i+1){s.c_w(c,h,e);s.c_w(c+'_"
+"s',o,a);}}else{s.c_w(c,h,e);g=s.c_r(c+'_s');s.c_w(c+'_s',g,a);}}f=s"
+".c_r(c+'_s');if(f.length==0)return j;else if(f!=k&&f!=l&&f!=m&&f!=n"
+"&&f!=o)return'';else return f;");
...
s.visitorNamespace="tribuneinteractive"
s.trackingServer="m.trb.com"
s.trackingServerSecure="sm.trb.com"
s.visitorMigrationKey="4D4DDB8E"
